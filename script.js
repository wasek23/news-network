...
const displayNews = (cat, data) => {
    ...
    newsListEl.innerHTML = data.map(news => {
        const { image_url, title, details, author, total_view, rating } = news;
        const { img, name, published_date } = author || {};
        console.log(news);

        return `<div class='singleNews col-12 col-sm-6 col-lg-4'>
            ...
            <div class='author d-flex'>
                ${img ? `<img src='${img}' class='img-fluid' alt='${name}' />` : 'No data found!'}

                <span>
                    <span class='authorName'>${name || 'No data found!'}</span>
                    <span class='publishDate'>${published_date || 'No data found!'}</span>
                </span>
            </div>
